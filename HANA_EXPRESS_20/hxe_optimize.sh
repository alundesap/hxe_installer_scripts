...
promptNewPwd() {
	...
	while [ 1 ] ; do
		...
		CRACK_MESSAGE=`/usr/sbin/cracklib-check <<EOF
${pwd}
EOF`
		CRACK_MESSAGE=`awk 'BEGIN { FS = ": " } ; {print $2}' <<EOF
$CRACK_MESSAGE
EOF`

		if [ "${CRACK_MESSAGE}" != "OK" ] ; then
			echo
			echo "Invalid password: $CRACK_MESSAGE"
			echo
			continue
		fi
...
postProcessXSA() {
	...
	if ! echo "$org_users" | grep ^OrgManager | grep XSA_ADMIN >& /dev/null; then
		xs set-org-role XSA_ADMIN HANAExpress OrgManager
		if [ $? -ne 0 ]; then
			exit 1
		fi
	fi
...
EOF
				if [ $? -ne 0 ]; then
					echo
					echo "Failed to register ${HXE_SID} database HDB resource with Cockpit."
					status=1
				fi
			fi
		fi
...
#	done
	...
	output=`hdbcons -e hdbnameserver "log release"`
	if echo "$output" | grep -i error >& /dev/null; then
		echo "${output}"
	fi
}
...
# Prompt new telemetry technical and XSA user/password if XSA is installed
if [ $HAS_XSA -eq 1 ]; then
